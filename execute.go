...
func ExecuteDockerVersion() error {
	return Execute("docker", "--version")
}

func ExecuteDockerBuild(dockerFile, imageName string) error {
	return Execute("docker", "build", "-t", imageName, "-f", dockerFile, ".")
}

func ExecuteDockerTag(imageName string, imageNameAlt string) error {
	return Execute("docker", "tag", imageName, imageNameAlt)
}

func ExecuteDockerPush(imageName string, configDir string) error {
	return Execute("docker", "--config", configDir, "push", imageName)
}

func ExecuteDockerRemoveImage(imageName string) error {
	return Execute("docker", "rmi", imageName)
}

func ExecuteKubectlVersion(kubeconfig string) error {
	return ExecuteWithRetries(KubectlPatchRetries, "kubectl", "--kubeconfig", kubeconfig,
		"version")
}

func ExecuteKubectlPatch(kubeconfig, namespace, workload, workloadType, patch string) error {
	return ExecuteWithRetries(KubectlPatchRetries, "kubectl", "--kubeconfig", kubeconfig,
